...
import core.stdc.math;       // Required for: sinf(), asinf(), cosf(), acosf(), sqrtf(), fabsf()
import core.stdc.float_;      // Required for: FLT_EPSILON
...
enum SMOOTH_CIRCLE_ERROR_RATE =  0.5f;
...
void DrawLineEx(Vector2 startPos, Vector2 endPos, float thick, Color color)
{
    ...
    if ((length > 0) && (thick > 0))
    {
        ...
        DrawTriangleStrip(strip.ptr, 4, color);
    }
}

enum BEZIER_LINE_DIVISIONS =         24;   // Bezier line divisions
...
void DrawLineBezier(Vector2 startPos, Vector2 endPos, float thick, Color color)
{
    ...
    for (int i = 1; i <= BEZIER_LINE_DIVISIONS; i++)
    {
        ...
        current.y = EaseCubicInOut(cast(float)i, startPos.y, endPos.y - startPos.y, cast(float)BEZIER_LINE_DIVISIONS);
        current.x = previous.x + (endPos.x - startPos.x)/ cast(float)BEZIER_LINE_DIVISIONS;
...
{
    DrawCircleV(Vector2( cast(float)centerX, cast(float)centerY ), radius, color);
}
...
void DrawCircleSector(Vector2 center, float radius, float startAngle, float endAngle, int segments, Color color)
{
    ...
    if (segments < minSegments)
    {
        ...
        segments = cast(int)((endAngle - startAngle)*ceilf(2*PI/th)/360);

        if (segments <= 0) segments = minSegments;
    }

    float stepLength = (endAngle - startAngle)/cast(float)segments;
    float angle = startAngle;

version (SUPPORT_QUADS_DRAW_MODE) {
...
void DrawCircleSectorLines(Vector2 center, float radius, float startAngle, float endAngle, int segments, Color color)
{
    ...
    if (cast(int)(endAngle - startAngle)%360 == 0) { limit = 2*segments; showCapLines = false; }
...
void DrawCircleGradient(int centerX, int centerY, float radius, Color color1, Color color2)
{
    ...
    rlBegin(RL_TRIANGLES);
        for (int i = 0; i < 360; i += 10)
        {
            ...
            rlVertex2f(cast(float)centerX, cast(float)centerY);
            rlColor4ub(color2.r, color2.g, color2.b, color2.a);
            rlVertex2f(cast(float)centerX + sinf(DEG2RAD*i)*radius, cast(float)centerY + cosf(DEG2RAD*i)*radius);
            rlColor4ub(color2.r, color2.g, color2.b, color2.a);
            rlVertex2f(cast(float)centerX + sinf(DEG2RAD*(i + 10))*radius, cast(float)centerY + cosf(DEG2RAD*(i + 10))*radius);
        }
    rlEnd();
}
...
void DrawEllipse(int centerX, int centerY, float radiusH, float radiusV, Color color)
{
    ...
    rlBegin(RL_TRIANGLES);
        for (int i = 0; i < 360; i += 10)
        {
            rlColor4ub(color.r, color.g, color.b, color.a);
            rlVertex2f(cast(float)centerX, cast(float)centerY);
            rlVertex2f(cast(float)centerX + sinf(DEG2RAD*i)*radiusH, cast(float)centerY + cosf(DEG2RAD*i)*radiusV);
            rlVertex2f(cast(float)centerX + sinf(DEG2RAD*(i + 10))*radiusH, cast(float)centerY + cosf(DEG2RAD*(i + 10))*radiusV);
        }
    rlEnd();
}
...
void DrawRingLines(Vector2 center, float innerRadius, float outerRadius, float startAngle, float endAngle, int segments, Color color)
{
    ...
    if (cast(int)(endAngle - startAngle)%360 == 0) { limit = 4*segments; showCapLines = false; }
...
{
    DrawRectangleV(Vector2( cast(float)posX, cast(float)posY ), Vector2( cast(float)width, cast(float)height ), color);
}
...
{
    DrawRectangleGradientEx(Rectangle( cast(float)posX, cast(float)posY, cast(float)width, cast(float)height ), color1, color2, color2, color1);
}
...
{
    DrawRectangleGradientEx(Rectangle( cast(float)posX, cast(float)posY, cast(float)width, cast(float)height ), color1, color1, color2, color2);
}
...
void DrawRectangleRounded(Rectangle rec, float roundness, int segments, Color color)
{
    ...
    if (segments < 4)
    {
        // Calculate the maximum angle between segments based on the error rate (usually 0.5f)
        float th = acosf(2*powf(1 - SMOOTH_CIRCLE_ERROR_RATE/radius, 2) - 1);
        segments = cast(int)(ceilf(2*PI/th)/4.0f);
        if (segments <= 0) segments = 4;
    }

    float stepLength = 90.0f/cast(float)segments;
    ...
    */
    ...
    const(Vector2)[12] point = [
        {cast(float)rec.x + radius, rec.y}, {cast(float)(rec.x + rec.width) - radius, rec.y}, { rec.x + rec.width, cast(float)rec.y + radius },     // PO, P1, P2
        {rec.x + rec.width, cast(float)(rec.y + rec.height) - radius}, {cast(float)(rec.x + rec.width) - radius, rec.y + rec.height},           // P3, P4
        {cast(float)rec.x + radius, rec.y + rec.height}, { rec.x, cast(float)(rec.y + rec.height) - radius}, {rec.x, cast(float)rec.y + radius},    // P5, P6, P7
        {cast(float)rec.x + radius, cast(float)rec.y + radius}, {cast(float)(rec.x + rec.width) - radius, cast(float)rec.y + radius},                   // P8, P9
        {cast(float)(rec.x + rec.width) - radius, cast(float)(rec.y + rec.height) - radius}, {cast(float)rec.x + radius, cast(float)(rec.y + rec.height) - radius} // P10, P11
    ];

    const(Vector2)[4] centers = [ point[8], point[9], point[10], point[11] ];
    const(float)[4] angles = [ 180.0f, 90.0f, 0.0f, 270.0f ];
...
void DrawRectangleRoundedLines(Rectangle rec, float roundness, int segments, float lineThick, Color color)
{
    ...
    if (segments < 4)
    {
        // Calculate the maximum angle between segments based on the error rate (usually 0.5f)
        float th = acosf(2*powf(1 - SMOOTH_CIRCLE_ERROR_RATE/radius, 2) - 1);
        segments = cast(int)(ceilf(2*PI/th)/2.0f);
        if (segments <= 0) segments = 4;
    }
     ...
     P6 \\  *P19             P18*   // P3
         \\                        //
          \\ P13              P12 //
           P5 ================== P4
    */
    const(Vector2)[16] point = [
        {cast(float)rec.x + innerRadius, rec.y - lineThick}, {cast(float)(rec.x + rec.width) - innerRadius, rec.y - lineThick}, { rec.x + rec.width + lineThick, cast(float)rec.y + innerRadius }, // PO, P1, P2
        {rec.x + rec.width + lineThick, cast(float)(rec.y + rec.height) - innerRadius}, {cast(float)(rec.x + rec.width) - innerRadius, rec.y + rec.height + lineThick}, // P3, P4
        {cast(float)rec.x + innerRadius, rec.y + rec.height + lineThick}, { rec.x - lineThick, cast(float)(rec.y + rec.height) - innerRadius}, {rec.x - lineThick, cast(float)rec.y + innerRadius}, // P5, P6, P7
        {cast(float)rec.x + innerRadius, rec.y}, {cast(float)(rec.x + rec.width) - innerRadius, rec.y}, // P8, P9
        { rec.x + rec.width, cast(float)rec.y + innerRadius }, {rec.x + rec.width, cast(float)(rec.y + rec.height) - innerRadius}, // P10, P11
        {cast(float)(rec.x + rec.width) - innerRadius, rec.y + rec.height}, {cast(float)rec.x + innerRadius, rec.y + rec.height}, // P12, P13
        { rec.x, cast(float)(rec.y + rec.height) - innerRadius}, {rec.x, cast(float)rec.y + innerRadius} // P14, P15
    ];

    const(Vector2)[4] centers = [
        {cast(float)rec.x + innerRadius, cast(float)rec.y + innerRadius}, {cast(float)(rec.x + rec.width) - innerRadius, cast(float)rec.y + innerRadius}, // P16, P17
        {cast(float)(rec.x + rec.width) - innerRadius, cast(float)(rec.y + rec.height) - innerRadius}, {cast(float)rec.x + innerRadius, cast(float)(rec.y + rec.height) - innerRadius} // P18, P19
    ];
...
version (SUPPORT_QUADS_DRAW_MODE) {
        ...
        rlBegin(RL_QUADS);
            for (int i = 0; i < sides; i++)
            {
                ...
                centralAngle += 360.0f/cast(float)sides;
                rlTexCoord2f((texShapesRec.x + texShapesRec.width)/texShapes.width, texShapesRec.y/texShapes.height);
                rlVertex2f(sinf(DEG2RAD*centralAngle)*radius, cosf(DEG2RAD*centralAngle)*radius);
            }
        rlEnd();
...
void DrawPolyLinesEx(Vector2 center, int sides, float radius, float rotation, float lineThick, Color color)
{
    if (sides < 3) sides = 3;
    float centralAngle = 0.0f;
    float exteriorAngle = 360.0f/cast(float)sides;
...
{
    int recCenterX = cast(int)(rec.x + rec.width/2.0f);
    int recCenterY = cast(int)(rec.y + rec.height/2.0f);

    float dx = fabsf(center.x - cast(float)recCenterX);
    float dy = fabsf(center.y - cast(float)recCenterY);
...
bool CheckCollisionLines(Vector2 startPos1, Vector2 endPos1, Vector2 startPos2, Vector2 endPos2, Vector2* collisionPoint)
{
    ...
    if (fabsf(div) < float.epsilon) return false;
    ...
    if (fabsf(startPos1.x - endPos1.x) > float.epsilon && (xi < fminf(startPos1.x, endPos1.x) || xi > fmaxf(startPos1.x, endPos1.x))) return false;
    if (fabsf(startPos2.x - endPos2.x) > float.epsilon && (xi < fminf(startPos2.x, endPos2.x) || xi > fmaxf(startPos2.x, endPos2.x))) return false;
    if (fabsf(startPos1.y - endPos1.y) > float.epsilon && (yi < fminf(startPos1.y, endPos1.y) || yi > fmaxf(startPos1.y, endPos1.y))) return false;
    if (fabsf(startPos2.y - endPos2.y) > float.epsilon && (yi < fminf(startPos2.y, endPos2.y) || yi > fmaxf(startPos2.y, endPos2.y))) return false;
